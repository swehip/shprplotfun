...
#' @param x_breaks_end  Break end, default for 100000. Works for all count values below that.
...
#' @examples
#' # Creating data
#' df <- data.frame(age = rpois(100000, 65), gender = sample(c('Woman', 'Woman', 'Man'), 100000, replace = TRUE))
#'
#' # Age pyramid
#' age_pyramid(df = df, age_var = 'age', gender_var = 'gender', man_level = 'Man', title = "This is an age pyramid")
#'
#' # Age pyramid with percent = FALSE
#' age_pyramid(df = df, age_var = 'age', gender_var = 'gender', man_level = 'Man', percent = FALSE, x_breaks = 5000, title = "This is an age pyramid")
#' @export
age_pyramid <- function(df, age_var = "Alder", gender_var = "Kon", man_level = "Man",
                        age_breaks = c(0,39,44,49,54,59,64,69,74,79,84, Inf),
                        age_labels = c("0-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80-84", "85+"),
                        percent = TRUE, x_breaks = 5, x_breaks_end = 100000,
                        title = "", subtitle = NULL, title_size = 14, subtitle_size = 10, title_margin = 1,
                        ...
                        text_size = 8, windows_font = "Calibri",
                        fill_colors = c("#377EB8", "#E41A1C"),
                        legend_pos = c(0,0), legend_labels = waiver(), label_breaks = waiver(),
  ...
  df$Population[df[,gender_var] == man_level] <- -1 * df$Population[df[,gender_var] == man_level]

  if(percent){
    df$Population[df[,gender_var] == man_level] <- df$Population[df[,gender_var] == man_level] / n_man * 100
    df$Population[df[,gender_var] != man_level] <- df$Population[df[,gender_var] != man_level] / n_woman * 100

    x_lim <- max(abs(df$Population))

    x_labels <- paste0(as.character(c(seq(x_breaks_end, 0, -x_breaks), seq(x_breaks, x_breaks_end, x_breaks))), "%")
  }else{
    x_labels <- c(seq(x_breaks_end, 0, -x_breaks), seq(x_breaks, x_breaks_end, x_breaks))
  }
  ...
  pyramid <- ggplot(df, aes_string(x = age_var, y = "Population", fill = gender_var)) +
    scale_fill_manual(paste0("n = ",as.character(n_woman + n_man)), values = fill_colors, labels = legend_labels, breaks = label_breaks, guide = guide_legend(nrow = legend_row, ncol = legend_col)) +
    theme_classic() +
    xlab(x_lab) +
    ylab(y_lab) +
    ggtitle(title, subtitle = subtitle) +
    geom_bar(stat = "identity", color = "black", size = contour_line_size) +
    scale_y_continuous(breaks = seq(-x_breaks_end, x_breaks_end, x_breaks),
                       labels = x_labels, limits = c(-x_lim,x_lim)) +
    coord_flip() +
