...
#'
#' Following variables MUST be included in the data set to be able to use the function:
#'
#' PREP_HipPain
#'
#' PREP_VASHealth
...
#' PREP_Charnley
#'
#' where eq10, eq20, eq30, eq40 and eq50 are the five numbers extracted using eq5d_3l() and eq5d0swe is obtained using
#' eq10, eq20, eq30, eq40, and eq50 in eqindSwed().
#' @param outcome Linear model outcome variable.
#' @param data1 Data frame containing data for the first year period.
#' @param data2 Data frame containing data for the second year period.
#' @param data3 Data frame containing data for the third year period.
#' @param data4 Data frame containing data for the fourth year period.
#' @param year_period1 First year period.
#' @param year_period2 Second year period.
#' @param year_period3 Third year period.
#' @param year_period4 Fourth year period.
...
expected <- function(outcome, data1, data2, data3, data4,
                     year_period1 = "2008_2009", year_period2 = "2010_2011", year_period3 = "2012_2013", year_period4 = "2014_2015",
                     mean_group = "Unit"){
...
frml <- as.formula(sprintf("%s ~   PREP_HipPain    +
                                   ...
                                   P_DiaGrp +
                                   P_Age + P_Gender + PREP_Charnley", outcome))
...
fited.means  <- data.frame( res2008_2009 = tapply(fit1$fitted.values, data1[[mean_group]], mean),
...
names(fited.means) <- c(paste0("res", year_period1),
