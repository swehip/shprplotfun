...
#'
#' Easy way to add PROM data to operation data. Preoperation and postoperation 1 year, 6 years and 10 years are added.
#' Also an option to add reoperation PROM.
#'
#' @param dataOp Corresponds to dataOperations in SHPR datalayer. Variables needed are SubjectKey, P_Side, P_ProstType and P_SurgDate.
#' @param dataPREP Corresponds to dataPROMBefore. Variables SubjectKey, PREP_HipPain[L|R] must be included.
#' @param dataPOSTP Corresponds to dataPROMAfter. Variables SubjectKey, POSTP_HipPain[L|R], POSTP_Satisfaction, POSTP_Satisfaction[L|R] must be included.
#' @param cens_date Censor date for PROM, example if cens_date = '2018-12-31' there will be no PROMS after that date.
#' @param primary_prom_start_year Start year for PROM, should probably not be changed.
#' @param add_reop_prom Boolean if reoperation PROM should be added, that PROM is marked with _reop after every variable.
#' @param reop_prom_start_year Start year for reop PROM, should probably not be changed.
#'
...
add_prom <- function(dataOp,
  ...
  # Tried to make it work without these variables included but no time, these are used for this
  ...
  postp_vars <-
    dplyr::intersect(names(dataPOSTP), pain_and_satis_vars)
  prep_vars <-
    dplyr::intersect(names(dataPREP), pain_and_satis_vars)
  satis_vars <-
    dplyr::intersect(
      names(dataPOSTP),
      c(
  ...
  if (postp_satis_factor) {
    ...
    dataPOSTP$POSTP_Satisfaction <-
      as.numeric(dataPOSTP$POSTP_Satisfaction)
  ...
  if (postp_satisR_factor) {
    ...
    dataPOSTP$POSTP_SatisfactionR <-
      as.numeric(dataPOSTP$POSTP_SatisfactionR)
  ...
  operations <- dataOp %>%
    ...
    dplyr::mutate(prim_time = as.numeric(difftime(P_SurgDate, R_SurgDate, units = "days")),
                  prim_time = case_when(prim_time >= 0 ~ prim_time)) %>%
  ...
  dataPOSTP <- dataPOSTP %>%
    dplyr::filter(POSTP_Date <= DateOfDeath |
                    is.na(DateOfDeath)) %>%
  ...
  dataOp <-
    left_join(dataOp, pre_prom, by = c("SubjectKey", "P_SurgDate"))
  ...
  post_prom1 <- post_prom %>%
    ...
    dplyr::mutate_at(satis_vars, list( ~ case_when(
      reop_time > reg_time |
        is.na(reop_time) ~ .
    ))) %>%
    dplyr::select(-prim_time,-reg_time,-reop_time)
  ...
  dataOp <-
    left_join(
      dataOp,
      post_prom1,
      by = c("SubjectKey" = "SubjectKey_1yr",
  ...
  dataOp <-
    left_join(
      dataOp,
      post_prom10,
      by = c("SubjectKey" = "SubjectKey_10yrs",
  ...
  if (add_reop_prom) {
    ...
    reops <- dplyr::mutate(reops,
      ...
      dplyr::mutate(
        reop_time_R = case_when(reop_time_R >= 0 ~ reop_time_R),
        reop_time_L = case_when(reop_time_L >= 0 ~ reop_time_L)
      ) %>%
      dplyr::mutate(reop_time = pmin(reop_time_R, reop_time_L, na.rm = TRUE)) %>%
    ...
    all_reops <- reops %>%
      ...
      dplyr::mutate(R_SurgDate2 = lead(R_SurgDate)) %>%
      dplyr::ungroup() %>%
    ...
    if (lubridate::year(Sys.Date()) >= 2023) {
      ...
      dataOp <- dataOp %>%
        dplyr::mutate(
          POSTP_HipPain_reop6yrs = case_when(
            P_Side == 1 ~ POSTP_HipPainR_reop6yrs,
            P_Side == 2 ~ POSTP_HipPainL_reop6yrs
          ),
          POSTP_Satisfaction_reop6yrs = case_when(
    ...
    if (lubridate::year(Sys.Date()) >= 2027) {
      ...
      dataOp <- dataOp %>%
        dplyr::mutate(
          POSTP_HipPain_reop10yrs = case_when(
            P_Side == 1 ~ POSTP_HipPainR_reop10yrs,
            P_Side == 2 ~ POSTP_HipPainL_reop10yrs
          ),
          POSTP_Satisfaction_reop10yrs = case_when(
  ...
  if (prosttype_factor) {
    dataOp$P_ProstType <-
      factor(dataOp$P_ProstType, labels = prosttype_levels)
