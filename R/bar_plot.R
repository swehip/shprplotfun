...
#' @param df  Data frame.
#' @param x_var  Variable for x axis, use string name. Recommended that x_var is in character in df.
#' @param fill_var  Variable for the different colors in bars, use string name. Use NULL if only one color for bars.
#' @param y_var  Variable for y axis, if NULL, count is used.
#' @param style  3 different styles of bar plots, stack, fill, or dodge. fill requires y_percent  TRUE.
#' @param group_by_x_var  Only relevant for style dodge. Boolean indicating if percentages should be for x_var or fill_var.
#' @param y_percent  If TRUE, y axis is in percent form. Otherwise in count form.
#' @param y_lim  Limit on y axis.
#' @param y_breaks  Length between each break on y axis.
#' @param y_breaks_end  Break end, default for 100000. Works for all count values below that.
...
#' @param title_margin  Distance between subtitle and title in pt. If no subtitle, title_margin  0.5*title_size.
...
#' @param legend_pos  Position of the legend in plot, if c(1,1), c(1,0) etc, legend inside plot.
...
#' @export
bar_plot <- function(df, x_var, fill_var = NULL, y_var = NULL, style = c("stack", "fill", "dodge")[1], group_by_x_var = TRUE,
                     y_percent = TRUE, y_lim = NULL, y_breaks = 2000, x_breaks = NULL,
                     y_breaks_end = 100000, title = NULL, subtitle = NULL, title_size = 14, subtitle_size = 10,
                     title_margin = 1, y_lab = NULL, x_lab = NULL, background_color = "moccasin",
                     panel_grid_color = "grey", panel_grid_size = 0.3, contour_line_size = 0.1, axis_size = 0.3, axis_text_angle = 0,
                     text_size = 8, fill_colors = c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00", "#FFFF33",
                                                    "#A65628", "#F781BF"),
                     legend_pos = "bottom", legend_labels = waiver(), label_breaks = waiver(),
  ...
  if(is.character(y_var)){
    ...
    if(!is.character(fill_var)){
      ...
      if(group_by_x_var){
        ...
        df <- df %>% group_by_(x_var, fill_var) %>% summarise(y = n()) %>% group_by_(x_var) %>% mutate(y2 = sum(y))

      }else{

        df <- df %>% group_by_(x_var, fill_var) %>% summarise(y = n()) %>% group_by_(fill_var)%>% mutate(y2 = sum(y))
  ...
  bars <- ggplot(data = df) +
    theme_classic() +
    scale_fill_manual(values = fill_colors, labels = legend_labels, breaks = label_breaks, guide = guide_legend(nrow = legend_row, ncol = legend_col)) +
    ylab(y_lab) +
    xlab(x_lab) +
  ...
  if(y_percent){
    ...
    if(style == "dodge"){
      bars <- bars + geom_bar(width=0.5, mapping = aes_string(x = x_var, y = "y/y2", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_dodge(width=0.5), color = "black", size = contour_line_size) +
        scale_y_continuous(label = scales::percent, breaks = seq(0, 1, by = y_breaks), limits = y_lim)

    }else if(style == "fill"){

      bars <- bars + geom_bar(width = 0.5, mapping = aes_string(x = x_var, y = "y/sum(y)", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_fill(vjust = 0.5, reverse = TRUE), color = "black", size = contour_line_size) +
        scale_y_continuous(label = scales::percent, breaks = seq(0, 1, by = y_breaks), limits = y_lim)
    }else{

      bars <- bars + geom_bar(width = 0.5, mapping = aes_string(x = x_var, y = "y/sum(y)", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_stack(vjust = 0.5, reverse = TRUE), color = "black", size = contour_line_size) +
        scale_y_continuous(label = scales::percent, breaks = seq(0, 1, by = y_breaks), limits = y_lim)

    }

  }else if(style == "fill"){
    bars <- bars + geom_bar(width = 0.5, mapping = aes_string(x = x_var, y = "y", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_fill(vjust = 0.5, reverse = TRUE), color = "black", size = contour_line_size) +
      scale_y_continuous(breaks = seq(0, y_breaks_end, by = y_breaks), limits = y_lim)

  }else if(style == "dodge"){
    bars <- bars + geom_bar(width = 0.5, mapping = aes_string(x = x_var, y = "y", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_dodge(width=0.5), color = "black", size = contour_line_size) +
      scale_y_continuous(breaks = seq(0, y_breaks_end, by = y_breaks), limits = y_lim)


  }else{

    bars <- bars + geom_bar(width = 0.5, mapping = aes_string(x = x_var, y = "y", fill = fill_var), stat = "identity", show.legend = show_legend, position = position_stack(vjust = 0.5, reverse = TRUE), color = "black", size = contour_line_size) +
      scale_y_continuous(breaks = seq(0, y_breaks_end, by = y_breaks), limits = y_lim)
  ...
  if(is.numeric(df[[x_var]]) & !is.null(x_breaks)){
    ...
    bars <- bars + scale_x_continuous(breaks = seq(floor(min(df[[x_var]])), ceiling(max(df[[x_var]])), by = x_breaks))
