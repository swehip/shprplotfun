...
#'   The year variables will be renamed as they appear in the data set.
...
#'   corresponds to a legend position is recommended, if length(subset) > 1.
...
prom_trends <-
  function(eq_vas_exp,
    ...
    eq_vas <-
      rbind(
        ggp_suit(eq_vas_exp, year = year, sheet = legend_labels[1]),
        ggp_suit(eq_vas_obs, year = year, sheet = legend_labels[2]),
        ggp_suit(eq_vas_riket, year = year, sheet = legend_labels[3])
      )
    pain <-
      rbind(
        ggp_suit(pain_exp, year = year, sheet = legend_labels[1]),
        ggp_suit(pain_obs, year = year, sheet = legend_labels[2]),
        ggp_suit(pain_riket, year = year, sheet = legend_labels[3])
      )
    satis <-
      rbind(
        ggp_suit(satis_exp, year = year, sheet = legend_labels[1]),
        ggp_suit(satis_obs, year = year, sheet = legend_labels[2]),
    ...
    get_ylim <- function(y_vec, y_break) {
      ...
      if (top - bottom < 3 * y_break) {
        top <-
          (((
            max(y_vec, na.rm = TRUE) + min(y_vec, na.rm = TRUE)
    ...
    for (i in subset) {
      eq_vas_data <-
        filter(eq_vas, hospital == hospitals[i] | hospital == riket_name)
      pain_data <-
        filter(pain, hospital == hospitals[i] | hospital == riket_name)
      satis_data <-
        filter(satis, hospital == hospitals[i] | hospital == riket_name)
