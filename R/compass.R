...
#' @param alpha_riket Modify color transparency for Swedish average color. Use < 1 if colors are to be mixed.
#' @param alpha_hosp Modify color transparency for hospital color. Use < 1 if colors are to be mixed.
#' @param titles  Character vector containing compass titles. If long titles, use '\n' to end line!
...
#'
#' @export
compass <- function(df, riket_name = "Riket", ncol = 6, riket_color = "#D95F02", hosp_color = "#1B9E77",
                    alpha_riket = 0.6, alpha_hosp = 0.6, titles = NULL, title_size = 8, line_size = 0.1){
  ...
  new_df$category <- ordered(new_df$category, levels = names(df)[-1])
  ...
  filtered_df$enhet <- ordered(filtered_df$enhet, levels = unique(filtered_df$enhet))
  ...
  compasses <- ggplot(data = filtered_df, aes(x = category, y = norm_value)) +
    coord_radar(start = -pi/length(names(df)[-1])) +
    theme_minimal() +
    facet_wrap(~ enhet, ncol = ncol, labeller = as_labeller(hospital_names)) +
    geom_polygon(data = riket_df, aes(group = riket), fill=riket_color, show.legend = FALSE, alpha = alpha_riket) +
    geom_polygon(aes(group = enhet), fill=hosp_color, show.legend = FALSE, alpha = alpha_hosp) +
    xlab(NULL) +
    ylab(NULL) +
