...
survSummary <- function(surv_object, times) {
  ...
  for (i in seq_along(surv_object$models)) {
    ...
    n[i] = ifelse(is.null(summary(surv_object$models[[i]], times, extend = TRUE)$n),
                  NA,
                  summary(surv_object$models[[i]], times, extend = TRUE)$n)


    n_risk[i] = ifelse(is.null(summary(surv_object$models[[i]], times, extend = TRUE)$n.risk),
                       NA,
                       summary(surv_object$models[[i]], times, extend = TRUE)$n.risk)

    surv_prob[i] = ifelse(is.null(summary(surv_object$models[[i]], times, extend = TRUE)$surv),
                          NA,
                          summary(surv_object$models[[i]], times, extend = TRUE)$surv)


    lower[i] = ifelse(is.null(summary(surv_object$models[[i]], times, extend = TRUE)$lower),
                      NA,
                      summary(surv_object$models[[i]], times, extend = TRUE)$lower)


    upper[i] = ifelse(is.null(summary(surv_object$models[[i]], times, extend = TRUE)$upper),
                      NA,
                      summary(surv_object$models[[i]], times, extend = TRUE)$upper)

    sd[i] <- ifelse(is.null(summary(surv_object$models[[i]], times)$std.err),
                    NA,
                    summary(surv_object$models[[i]], times)$std.err)

  }

  result <- data.frame(antal = n,
                       at_risk = n_risk,
                       surv_prob = surv_prob,
                       lower = lower,
                       upper = upper,
  ...
  result <- dplyr::bind_cols(group_vars, result)
