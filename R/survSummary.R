...
survSummary <- function(surv_object, times) {
  ...
  for (i in seq_along(surv_object$models)) {
    n[i] <- ifelse(
      is.null(summary(surv_object$models[[i]], times, extend = TRUE)$n),
                  NA,
                  summary(surv_object$models[[i]], times, extend = TRUE)$n)


    n_risk[i] <- ifelse(
      is.null(summary(surv_object$models[[i]], times, extend = TRUE)$n.risk),
      NA,
      summary(surv_object$models[[i]], times, extend = TRUE)$n.risk
    )

    surv_prob[i] <- ifelse(
      is.null(summary(surv_object$models[[i]], times, extend = TRUE)$surv),
      NA,
    ...
    lower[i] <- ifelse(
      is.null(summary(surv_object$models[[i]], times, extend = TRUE)$lower),
      NA,
    ...
    upper[i] <-  ifelse(
      is.null(summary(surv_object$models[[i]], times, extend = TRUE)$upper),
      NA,
      summary(surv_object$models[[i]], times, extend = TRUE)$upper
    )

    sd[i] <-
      ifelse(is.null(summary(surv_object$models[[i]], times)$std.err),
             NA,
